package epk.sam.ebooks.corejava9.v2.ch08.chart;
...
/**
 * The panel inside the DoubleArrayEditor. It contains a list of the array
 * values, together with buttons to resize the array and change the currently
 * selected list value.
 * 
...
public class DoubleArrayEditorPanel extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2205816517384088800L;
	private PropertyEditorSupport editor;
	private double[] array;
	private JFormattedTextField sizeField = new JFormattedTextField(
			new Integer(0));
	...
	public DoubleArrayEditorPanel(PropertyEditorSupport ed) {
		...
		add(sizeField,
				new GBC(0, 0, 1, 1).setWeight(100, 0).setFill(GBC.HORIZONTAL));
		add(valueField,
				new GBC(0, 1, 1, 1).setWeight(100, 0).setFill(GBC.HORIZONTAL));
		...
		add(new JScrollPane(elementList),
				new GBC(0, 2, 2, 1).setWeight(100, 100).setFill(GBC.BOTH));
	...
	/**
	 * This method is called when the user wants to change the size of the
	 * array.
	 */
	...
	/**
	 * This method is called when the user wants to change the currently
...
class DoubleArrayListModel extends AbstractListModel<String> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6009911432500325713L;
	private double[] array;
	 ...
	 * 
	 * @param i
	 *            the index whose value to change
	 * @param value
