...
public class ChartBean extends JComponent {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4110510434778489839L;
	...
	public void paintComponent(Graphics g) {
		...
		double scale = (clientHeight - top - 2 * INSETS)
				/ (maxValue - minValue);
		y = clientHeight;
	...
	public void setValues(int i, double value) {
		if (0 <= i && i < values.length) {
			...
			for (PropertyChangeListener listener : getPropertyChangeListeners())
				listener.propertyChange(new IndexedPropertyChangeEvent(this,
						"values", oldValue, value, i));
		}
	}
